                ...
                if (descendants[j].nodeName == "NODEREF")
                {
                ...
                else if (descendants[j].nodeName == "LEAF")
                    {
                        ...
                        if (type == "patch") {
                            ...
                            console.log("patch attr");
                            var clines = descendants[j].children;
                            var attributes=[clines.length];
                            var cplines = [clines.length];
                            for (var k = 0; k < clines.length; k++) {
                                var cpoints = clines[k].children;
                                var cpointsA = [cpoints.length];
                                for(var cp = 0; cp < cpoints.length; cp++){
                                    var cpoint = cpoints[cp];
                                    //ver os atributos do cpoin
                                    ...
                                    cpointsA[cp]= [xx,yy,zz,ww];
                                }
                               cplines[k]=cpointsA.slice(0);
                            }
                            attributes= cplines;   
 
                            //verificar se cplines and cpoints foram bem lidos
         
                this.nodes[nodeID].addLeafs(new MyGraphLeaf(this.scene,descendants[j],type,attributes));
                        }
                        //parse leaf
                        if(type!="patch")
                          this.nodes[nodeID].addLeafs(new MyGraphLeaf(this.scene,descendants[j],type));
                        sizeChildren++;
                    }
                    else
...
MySceneGraph.prototype.displayScene = function() {
    ...
    this.nodes["root"].display(this.scene, "null", "null");
